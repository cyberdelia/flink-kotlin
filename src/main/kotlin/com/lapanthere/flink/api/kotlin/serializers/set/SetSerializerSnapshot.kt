...
public class SetSerializerSnapshot<T> : CompositeTypeSerializerSnapshot<Set<T>, SetTypeSerializer<T>> {
    ...
    override fun createOuterSerializerWithNestedSerializers(nestedSerializers: Array<TypeSerializer<*>>): SetTypeSerializer<T> {
        @Suppress("UNCHECKED_CAST")
        val elementSerializer = nestedSerializers[0] as TypeSerializer<T>
        return SetTypeSerializer(elementSerializer)
    }

    override fun getNestedSerializers(outerSerializer: SetTypeSerializer<T>): Array<TypeSerializer<*>> =
        arrayOf(outerSerializer.elementSerializer)
